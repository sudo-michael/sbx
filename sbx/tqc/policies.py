...
class TQCLagPolicy(BaseJaxPolicy):
    action_space: spaces.Box  # type: ignore[assignment]
    ...
    def build(self, key: jax.random.KeyArray, lr_schedule: Schedule, qf_learning_rate: float) -> jax.random.KeyArray:
        ...
        self.qfc1_state = RLTrainState.create(
            apply_fn=self.qfc.apply,
            params=self.qfc.init(
                {"params": qf1_key, "dropout": dropout_key1},
                obs,
                action,
            ),
            target_params=self.qfc.init(
                {"params": qf1_key, "dropout": dropout_key1},
                obs,
                action,
            ),
            tx=optax.adam(learning_rate=qfc_learning_rate),  # type: ignore[call-arg]
        )
        self.qfc2_state = RLTrainState.create(
            apply_fn=self.qfc.apply,
            params=self.qfc.init(
                {"params": qf2_key, "dropout": dropout_key2},
                obs,
            ...
            tx=self.optimizer_class(
                learning_rate=qfc_learning_rate,  # type: ignore[call-arg]
                **self.optimizer_kwargs,
            ),
        )
        self.actor.apply = jax.jit(self.actor.apply)  # type: ignore[method-assign]
        ...
        self.qfc.apply = jax.jit(  # type: ignore[method-assign]
            self.qfc.apply,
            static_argnames=("dropout_rate", "use_layer_norm"),
        )
