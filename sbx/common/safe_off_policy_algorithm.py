...
# Need to make own SafeOffPolicyAlgorithm in order to store cost to safe buffer
...
class SafeOffPolicyAlgorithm(BaseAlgorithm):
    ...
    :param env: The environment to learn from
                (if registered in Gym, can be str. Can be None for loading trained models) :param learning_rate: learning rate for the optimizer,
    ...
    def _store_transition(
        ...
        if self._vec_normalize_env is not None:
            ...
            assert False
            costs_ = self._vec_normalize_env.get_original_reward()
        else:
            # Avoid changing the original ones
            self._last_original_obs, new_obs_, reward_, costs_ = self._last_obs, new_obs, reward, costs
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            self._store_transition(replay_buffer, buffer_actions, new_obs, rewards, costs, dones, infos)
...
class SafeOffPolicyAlgorithmJax(SafeOffPolicyAlgorithm):
    def __init__(
        self,
    ...
    def _setup_model(self) -> None:
        ...
        self.qfc_learning_rate = self.qf_learning_rate or self.lr_schedule(1)
        self.set_random_seed(self.seed)
        # Make a local copy as we should not pickle
        # the environment when using HerReplayBuffer
        replay_buffer_kwargs = self.replay_buffer_kwargs.copy()
